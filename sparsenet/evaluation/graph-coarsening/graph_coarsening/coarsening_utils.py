...
from sparsenet.util.util import timefunc
...
def coarsen(
    ...
    # print('inside coarsen function:', n_target)
...
def get_proximity_measure(G, name, K=10):
    ...
    if "lanczos" in name:
        l_lan, X_lan = sp.sparse.linalg.da(G.L, k=K, which="SM", tol=1e-2)
    elif "cheby" in name:
    ...
    if name == "heavy_edge":
        # wmax = np.array(np.max(G.W, 0).todense())[0] + 1e-5 # edited by Chen to save memory
        wmax = np.array(np.max(G.W, 0)[0].todense())[0] + 1e-5
