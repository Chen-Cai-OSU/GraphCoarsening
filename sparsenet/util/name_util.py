...
from sparsenet.util.util import model_dir

loukas_datasets = ['minnesota', 'airfoil', 'yeast', 'bunny']

methods = ['variation_edges', 'variation_neighborhoods', 'algebraic_JC', 'heavy_edge' 'affinity_GS']  # , 'kron'

syn_graphs = ['random_geo', 'random_er', 'ws', 'ba', 'shape']

big_ego_graphs = ['PubMed', 'Coauthor-CS', 'Coauthor-physics', 'Amazon-photo', 'Amazon-computers', 'yelp', 'reddit', 'flickr']
ego_graphs = big_ego_graphs + ['CiteSeer']

from sparsenet.util.util import fig_dir, make_dir
import os
import os.path as osp

# def fig_name(args, name='eig'):
#     dir = os.path.join(fig_dir(), args.dataset, str(args.ratio), args.method, '')
#     make_dir(dir)
#     return os.path.join(dir, name + '.pdf')
